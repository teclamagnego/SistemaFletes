...
@section('content')
<h2><i class="bi bi-pencil-square"></i> Editar Transportista: {{ $carrier->full_name }}</h2>
<div class="card mt-3">
    <div class="card-body">
        <form action="{{ route('carriers.update', $carrier) }}" method="POST">@csrf @method('PUT')
            <div class="row">
                <div class="col-md-4 mb-3"><label class="form-label">Nombre *</label><input type="text" name="nombre"
                        class="form-control @error('nombre') is-invalid @enderror"
                        value="{{ old('nombre', $carrier->nombre) }}" required>@error('nombre')<div
